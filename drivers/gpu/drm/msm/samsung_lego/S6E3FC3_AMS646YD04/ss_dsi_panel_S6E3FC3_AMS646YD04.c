...
*/
#include "ss_dsi_panel_S6E3FC3_AMS646YD04.h"
#include "ss_dsi_mdnie_S6E3FC3_AMS646YD04.h"
					...
					int *level_key, bool is_hbm, bool is_hmt)
{
	...
	if (ss_panel_revision(vdd) >= 'D') {
		if (cur_rr == 60) {
			vrr_cmds->cmds[0].ss_txbuf[1] = 0x00; /* 60 HZ */
			vrr_cmds->cmds[0].ss_txbuf[2] = 0x00; /* 60 HZ */

			vrr_cmds->cmds[2].ss_txbuf[1] = 0x04;

			vrr_cmds->cmds[4].ss_txbuf[1] = 0x80;

			vrr_cmds->cmds[6].ss_txbuf[1] = 0x22;
		} else {
			vrr_cmds->cmds[0].ss_txbuf[1] = 0x00; /* 90 HZ */
			vrr_cmds->cmds[0].ss_txbuf[2] = 0x04; /* 90 HZ */

			vrr_cmds->cmds[2].ss_txbuf[1] = 0xB4;

			vrr_cmds->cmds[4].ss_txbuf[1] = 0xC0;

			vrr_cmds->cmds[6].ss_txbuf[1] = 0x02;
		}
	} else  {
		if (cur_rr == 60) {
			vrr_cmds->cmds[0].ss_txbuf[1] = 0x00; /* 60 HZ */
							...
							(struct samsung_display_driver_data *vdd, int *level_key)
{
	...
	} else if (ss_panel_revision(vdd) >= 'E') {
		/* Smooth transition : 0x28 */
